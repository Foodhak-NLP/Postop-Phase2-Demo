...
with st.container(border=True):
    st.markdown("**Weekly adherence logic**")

    col1, col2 = st.columns([1, 4])
    with col1:
        st.markdown("**< 55%**")
    with col2:
        st.markdown("Step down to **Basic Recovery**")

    col1, col2 = st.columns([1, 4])
    with col1:
        st.markdown("**55%–79%**")
    with col2:
        st.markdown("Continue the current/basic plan")

    col1, col2 = st.columns([1, 4])
    with col1:
        st.markdown("**≥ 80%**")
    with col2:
        st.markdown("Step up to **Targeted Recovery**, only if currently in Basic Recovery and safety is clear")

    st.divider()

    st.caption(
        "Reviews are calculated at weekly checkpoints: Day 7, Day 14, and Day 21. "
        "At the end of Day 30, new biomarker results are expected, and CQL reruns "
        "to select the next action from the updated action space."
    )
